            ...
            <div class="wrapper">
                            ...
                            <grid-item style="overflow:auto; border: 2px dashed grey; border-radius: 2%;" :class="{classOverflow : item.type=='table'}" v-for="(item, index) in gridItems" :key="item.i"
                                ...
                                <template v-if="item.type == 'treemap'">
                                    <kendo-treemap :ref="'treemap'+item.i" :data-source="item.data" 
                                        :value-field="'value'" 
                                        :text-field="'name'" style="width: 100%; height: 100%"
                                        >
                                    </kendo-treemap>
                                </template>
                                <div v-if="item.type == 'sticker'" :style="{backgroundColor: item.color, height:'100%', width:'100%', textAlign: 'center'}">
                                    <p style="color: white">@{{item.symbol}}</p>
                                    <p style="color: white"><span>Open:</span>&nbsp @{{item.open}}</p>
                                    <p style="color: white"><span>High:</span>&nbsp@{{item.high}}</p>
                                    <p style="color: white"><span>Low:</span>&nbsp@{{item.low}}</p>
                                    <p style="color: white"><span>Close:</span>&nbsp@{{item.close}}</p>
                                </div>
                ...
                <modal name="launcher">
                    ...
                    <div v-else class="row">
                        ...
                        <div class="col-md-11" v-if="route == 'table'" style="margin-top: 3%">
                            <v-select multiple @search="onSearch" v-model="selectedSymbol" :filterable="false" :options="filteredSymbols"></v-select>
                            <button @click="addTable" class="btn btn-primary" style="margin-top: 5%; margin-left: 37%">Add</button>
                        </div>
